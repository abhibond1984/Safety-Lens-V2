...
''';
  ...
  /// Falls back to offline LocalAI ONLY if there's a network error.
  ...
  static Future<Map<String, dynamic>?> analyseImage(File imageFile) async {
    if (_apiKey == 'YOUR_GEMINI_API_KEY_HERE' || _apiKey.isEmpty) {
      // API key not configured — refuse to analyse to prevent fake results
      throw Exception(
        'Gemini API key not configured. Please paste your key in lib/services/gemini_vision.dart line 13.',
      );
    }
    ...
    try {
      ...
      // Network unreachable — fall back to offline rule-based AI
      final fallback = await LocalAI.analyseImage(imageFile);
      fallback['_source'] = 'offline_network_error';
      return fallback;
    } catch (e) {
      rethrow;
    }
